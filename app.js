...
function renderLimitedPhotoList(files,removeKind,removeArg){
  ...
  for(var i=0;i<visible;i++){
    ...
    var thumb=stored?('<img src="'+stored.dataUrl+'" alt="'+esc(f.name||'')+'">'):'<span class="photo-thumb-fallback">📷</span>';
    var more=(i===2&&files.length>3)?'<button class="photo-more" onclick="event.stopPropagation();openEvidenceGallery(\''+removeKind+'\',\''+removeArg+'\')">+'+(files.length-3)+'장</button>':'';
...
function evidenceFiles(kind,arg){
  var i=Number(arg);
  if(kind==='accidentBefore')return S.accidents[i]?.beforeFiles||[];
  if(kind==='accidentAfter')return S.accidents[i]?.afterFiles||[];
  if(kind==='taskBefore')return S.tasks[i]?.beforeFiles||[];
  if(kind==='taskAfter')return S.tasks[i]?.afterFiles||[];
  return [];
}
function openEvidenceGallery(kind,arg){
  var files=evidenceFiles(kind,arg),old=document.getElementById('evidenceGallery');if(old)old.remove();
  var box=document.createElement('div');box.id='evidenceGallery';box.className='img-zoom evidence-gallery';
  var items=files.map(function(f,i){var p=PHOTO_STORE.get(f.id);return '<div class="evidence-gallery-item">'+(p?'<img src="'+p.dataUrl+'" alt="'+esc(f.name||'')+'">':'<div class="photo-thumb-fallback">📷</div>')+'<span>'+(i+1)+' / '+files.length+'</span><button onclick="event.stopPropagation();removePhotoAt(\''+kind+'\',\''+arg+'\','+i+');document.getElementById(\'evidenceGallery\')?.remove()">삭제</button></div>'}).join('');
...
function removePhotoAt(kind,arg,i){
  ...
  if(kind==='accidentBefore'){S.accidents[Number(arg)].beforeFiles.splice(i,1);save();accident();return}
...
function restoreLegacyInspection(row){
  var b=S.basic;
  if(row.delivery)b.delivery=row.delivery;
  if(row.inboundBoxes!==''&&row.inboundBoxes!=null)b.inboundBoxes=String(row.inboundBoxes);
  if(row.inboundHelpers!==''&&row.inboundHelpers!=null)b.inboundHelpers=String(row.inboundHelpers);
  if(row.inboundStart)b.inboundStart=row.inboundStart;if(row.inboundEnd)b.inboundEnd=row.inboundEnd;
  if(row.inboundStaff!==''&&row.inboundStaff!=null)b.inboundStaff=String(row.inboundStaff);
  if(row.inboundHelperOutAt)b.inboundHelperOutAt=row.inboundHelperOutAt;
  if(row.floors)b.floors=String(row.floors);
  b.hasStairs=row.hasStairs==='유'?'유':'무';b.hasElevator=row.hasElevator==='유'?'유':'무';b.hasEscalator=row.hasEscalator==='유'?'유':'무';
  if(row.tbmConfirmMethod)S.tbm.confirmMethod=row.tbmConfirmMethod;
  if(row.tbmAmTime)S.tbm.amTime=row.tbmAmTime;if(row.tbmPmTime)S.tbm.pmTime=row.tbmPmTime;

  /* 구버전은 전체 답변을 저장하지 않았으므로 정상 항목은 양호 기본값으로 채우고,
     이슈상세의 선택 답변 문구가 일치하는 문항만 당시 미흡 선택으로 되돌린다. */
  ensureDefaults();
  S.workChecked=S.workChecked||{};
  D.works.forEach(function(_,wi){S.workChecked[wi]={checkedAt:row.date||'',checkedBy:row.inspector||'',status:'checked'}});
  var open=(S.store&&S.store.openIssues)||[];
  (row.issues||[]).forEach(function(x,xi){
    var files=(open[xi]&&open[xi].beforeFiles)||[],detail=String(x.detail||'').trim();
    if(x.category==='작업점검'){
      var wi=D.works.findIndex(function(w){return w[0]===x.item});
      if(wi<0)return;
      var questions=D.works[wi][1];
      for(var qi=0;qi<questions.length;qi++){
        var opts=questions[qi][1]||[],oi=opts.indexOf(detail);
        if(oi<0&&detail)oi=opts.findIndex(function(opt){return detail.indexOf(opt)>=0||opt.indexOf(detail)>=0});
        if(oi>=0){S.wa[wi][qi]={oi:oi,risk:(questions[qi][3]||[]).indexOf(oi)>=0,hazards:questions[qi][2]||[],files:files};break}
      }
      return;
    }
    if(x.category==='공통·시설'||x.category==='소방'||x.category==='TBM'){
      var kind=x.category==='공통·시설'?'common':(x.category==='소방'?'fire':'tbm');
      S[kind].issues.push({id:'legacy-'+row.inspectionId+'-'+xi,item:x.item||'미흡사항',note:detail,files:files});
      return;
    }
    if(x.category==='사다리'){
      var type=(D.ladderTypes||[]).find(function(t){return String(x.item||'').indexOf(t)===0})||'기타';
      var item=String(x.item||'').slice(type==='기타'?0:type.length).trim()||'이상사항';
      if(S.ladder.types.indexOf(type)<0)S.ladder.types.push(type);
      S.ladder.counts[type]=Math.max(1,Number(S.ladder.counts[type])||0);S.ladder.typeStatus[type]='bad';
      S.ladder.issues.push({id:'legacy-'+row.inspectionId+'-'+xi,typeKey:type,type:type,item:item,note:detail,files:files});
      return;
    }
    if(x.category==='기타사항')S.others.push({id:'legacy-'+row.inspectionId+'-'+xi,text:x.item||detail,files:files,task:false});
  });
  S.common.status=S.common.issues.length?'bad':'good';S.fire.status=S.fire.issues.length?'bad':'good';S.tbm.status=S.tbm.issues.length?'bad':'good';S.ladder.status=S.ladder.issues.length?'bad':'good';
}
...
function tasks(){
  ...
  if(!S.tasks.length){
    ...
    for(i=0;i<S.tasks.length;i++){
      ...
      h+='<button class="task-review-head" onclick="toggleTaskCard('+i+')"><span><small>'+esc(x.date||'-')+' · 이전 점검</small><b>'+esc(x.title)+'</b></span><span><i>'+esc(x.status||'조치대기')+'</i><strong>⌄</strong></span></button>';
      if(!expanded){h+='</div>';continue}
    ...
    if(sub==='current')h+='<div class="acc-sub-body"><div class="field"><label>현재 상태 <span class="req">*</span></label><textarea onchange="setTaskField('+i+',\'currentState\',this.value)" placeholder="이번 재점검에서 확인한 상태를 적어 주세요.">'+esc(x.currentState)+'</textarea></div><div class="acc-photo-box"><b>조치 전 사진 <small>이번 재점검의 현재 상태</small></b>'+renderLimitedPhotoList(x.beforeFiles,'taskBefore',i)+'</div></div>';
      ...
      if(sub==='action'){
      h+='<div class="acc-sub-body"><div class="grid"><div class="field"><label>책임구분</label><select onchange="setTaskField('+i+',\'owner\',this.value)">';
...
function completeTask(i){
  ...
  x.confirmed=true;x.sectionOpen='';S.taskOpenKey='';save();tasks();toast('조치 확인을 완료했습니다.');
}
...
function completionState(){
  ...
  if(hasOpenIssues()){
    syncTasks();
    const un=(S.tasks||[]).filter(function(x){return !x.confirmed});
    if(un.length)missing.push({kind:'tasks',label:`이전 미조치사항 재확인 · 미작성 ${un.length}건`});
  }
  return missing;
}
...
function getLandscapeReportSnapshot(){
  ...
  const accidents=(S.accidents||[]).map(x=>({
    ...
    actionText:x.actionText||'',beforePhotos:resolvePhotos(x.beforeFiles),afterPhotos:resolvePhotos(x.afterFiles),photos:resolvePhotos(x.afterFiles)
  }));
...
function loadReportCssOnce(){
  return new Promise((resolve,reject)=>{
    ...
    el.rel='stylesheet';el.href='report-v12.css';el.setAttribute('data-report-css','1');
    el.onload=()=>resolve();
    el.onerror=()=>{
      /* 배포 누락·캐시 문제에 대비해 기존 이름을 한 번 더 시도한다.
         둘 다 실패하면 무스타일 PDF를 만들지 말고 원인을 명확히 알린다. */
      el.onerror=()=>reject(new Error('결과보고서 디자인 파일(report-v12.css)을 불러오지 못했습니다. GitHub 배포 파일을 확인해 주세요.'));
      el.href='report.css?v=12';
    };
    document.head.appendChild(el);
  });
}
